...
public abstract class LayoutLeaveBehindCallback  extends ItemTouchHelper.SimpleCallback {
    ...
    public void onChildDrawOver(@NonNull Canvas c, @NonNull RecyclerView recyclerView,
        ...
        // Para que sólo afecte la vista foreground.
        getDefaultUIUtil().onDrawOver(c, recyclerView, getForegroundView(viewHolder), dX,
    ...
    public void onChildDraw(@NonNull Canvas c, @NonNull RecyclerView recyclerView,
        ...
        // Para que sólo afecte la vista foreground.
        getDefaultUIUtil().onDraw(c, recyclerView, getForegroundView(viewHolder), dX,
                dY, actionState, isCurrentlyActive);
        View rightLeaveBehindView = getRightLeaveBehindView(viewHolder);
        View leftLeaveBehindView = getLeftLeaveBehindView(viewHolder);
        if (dX < 0) {
            if (rightLeaveBehindView != null) {
                rightLeaveBehindView.setVisibility(View.VISIBLE);
            }
            if (leftLeaveBehindView != null) {
                leftLeaveBehindView.setVisibility(View.INVISIBLE);
            }
        } else if (dX > 0) {
            if (rightLeaveBehindView != null) {
                rightLeaveBehindView.setVisibility(View.INVISIBLE);
            }
            if (leftLeaveBehindView != null) {
                leftLeaveBehindView.setVisibility(View.VISIBLE);
            }
        } else {
            if (rightLeaveBehindView != null) {
                rightLeaveBehindView.setVisibility(View.INVISIBLE);
